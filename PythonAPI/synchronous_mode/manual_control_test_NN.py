...
sys.path.append('/home/ruihan/scenario_runner')
...
class World(object):
    def __init__(self, carla_world, hud, args,num_wps=20, nn_model_path='/home/ruihan/scenario_runner/models/MLP/mlp_model_long_states_Adam_wps20_lr0.001_bs128_optimAdam_f100.pth'):
    ...
    def restart(self):
        ...
        blueprint = random.choice(self.world.get_blueprint_library().filter("vehicle.lincoln*"))
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(location=carla.Location(x=-88.646866, y=-103.266113, z=0), rotation=carla.Rotation(pitch=-0.471802, yaw=89.843742, roll=0.000000))
            print("spawn_point", spawn_point)
            # self.world.debug.draw_string(spawn_point.location, 'OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO', draw_shadow=True,
            #                                    color=carla.Color(r=255, g=0, b=0), life_time=120000.0,
            #                                    persistent_lines=True)
            # self.world.debug.draw_point(spawn_point.location, 30, carla.Color(255,0,0), 120000.0)
...
class KeyboardControl(object):
    def __init__(self, world, start_in_autopilot, start_in_nn_controller=False):
...
# customized methods
# ===================================
    def get_nn_controller_wp(self, world, num_wps=50, sampling_radius=2.0, \
                             max_pos_val = 500, max_yaw_val = 180, max_speed_val = 40):
        num_wps = world.nn_model_num_wps
        cur_loc = world.player.get_transform().location
        ...
        cur_vel = world.player.get_velocity()

        speed = math.sqrt(cur_vel.x**2+cur_vel.y**2)/max_speed_val
        yaw = world.player.get_transform().rotation.yaw
        state = np.hstack((np.array([yaw, speed]),np.array([cur_loc.x, cur_loc.y])- np.array([cur_wp.transform.location.x, cur_wp.transform.location.y]), \
        ...
        # state = state.to(world.device)
        control = world.nn_model(state)
        control = control.data.cpu().numpy()[0]
        self._control.throttle = control[0].item()
        self._control.steer = control[1].item()
        self._control.brake = 0 # 1 if control[2].item()>0.5 else 0
...
def game_loop(args):
    ...
    try:
        ...
        # try to draw the waypoints
        # map = world.world.get_map()
        # distance = 1
        # waypoints = map.generate_waypoints(distance)
        # for w in waypoints:
        #     print("waypoint", w.transform.location)
        #     world.world.debug.draw_string(w.transform.location, 'X', draw_shadow=False,
        #                                        color=carla.Color(r=255, g=0, b=0), life_time=120.0,
        #                                        persistent_lines=True)
            # world.world.debug.draw_point(w.transform.location, size=3, color=(255,0,0), life_time=120000.0, persistent_lines=True)
        ...
        while True:
            ...
            # cur_loc = world.player.get_transform().location
            # cur_wp = map.get_waypoint(cur_loc)
            # print("waypoint", cur_wp)
            # world.world.debug.draw_string(cur_wp.transform.location, 'O', draw_shadow=False,
            #                                    color=carla.Color(r=255, g=0, b=0), life_time=120.0,
            #                                    persistent_lines=True)
            world.tick(clock)
