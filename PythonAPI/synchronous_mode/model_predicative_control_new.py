'''
Create a copy and adapt this to new version of Carla,
should work with synchronous_mode_client_control.py
'''
import random
import numpy as np
...
class MPCController(Controller):
    ...
    def control(self, future_wps, measurements, depth_array=None):
        # find the nearest waypoint, which_closest is the index, location is current np.array[x,y]
        # get the next a few pts look ahead
        # set the interval between adjacent wps (local)
        # TODO: adjust the params if necessary

        future_wps_np = []
        for future_wp in future_wps:
            future_wps_np.append(np.array([future_wp.transform.location.x, future_wp.transform.location.y]))
        future_wps_np = np.array(future_wps_np) # equivalent to pts in org mpc.py

        v = np.sqrt(measurements.v.x**2 + measurements.v.y**2)
        ψ = measurements.t.rotation.yaw

        cos_ψ = np.cos(ψ)
        sin_ψ = np.sin(ψ)

        x, y = measurements.t.location.x, measurements.t.location.y
        
        pts_car = MPCController.transform_into_cars_coordinate_system(future_wps_np, x, y, cos_ψ, sin_ψ)

        print("pts_car", pts_car.shape)
