...
def sigmoid_constrain(u, min_bounds, max_bounds):
    ...
    return diff * (1./ (1 + T.exp(-(u - mean))) - 0.5) + mean
...
def tensor_constrain(u, min_bounds, max_bounds):
    ...
    diff = (max_bounds - min_bounds) / 2.0
    mean = (max_bounds + min_bounds) / 2.0
    return diff * T.tanh(u - mean) + mean

def hard_sigmoid_constrain(u, min_bounds, max_bounds):
    """Constrains a control vector between given bounds through a squashing
    function.
    ...
    diff = (max_bounds - min_bounds) / 2.0
    mean = (max_bounds + min_bounds) / 2.0
    return mean + (T.nnet.hard_sigmoid((u - mean)/diff * 2.5) - 0.5) * diff / 0.5
