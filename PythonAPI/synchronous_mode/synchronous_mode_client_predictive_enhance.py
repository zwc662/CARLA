...
from ilqr.enhance import PolicyEnhancer
...
def get_enhance_control(measurements, controller, target_waypoints, nn_trajectory_pred, avoidances, verbose = False):
    ...
    # for ILQRController
    xs, us = controller.control(target_waypoints_np, nn_trajectory_pred, avoidances)
    ...
    if verbose or True:
        print("input from nn", nn_trajectory_pred.us[0])
        print("output from enhance_control", us[0])
...
def main():
    ...
    try:
        ...
        camera_rgb = world.spawn_actor(
            blueprint_library.find('sensor.camera.rgb'),
            carla.Transform(carla.Location(x=55, y =55, z=20), carla.Rotation(pitch=-55)))
        ...
        # Initialize the NN controllers from file 
        ...
        enhance_min_distance = 0.0
        enhance_target_waypoints = deque(maxlen=enhance_number_waypoints)
        ...
        with CarlaSyncMode(world, camera_rgb, camera_semseg, fps=30) as sync_mode:
            ...
            while True:
                ...
                enhance_min_distance = spd * enhancer.dt
                ...
                if not should_auto:
                    ...
                    if unsafe < ilqr_number_waypoints or no_interference:
                        ...
                        query_target_waypoints(current_waypoint, \
                                target_speed, enhance_number_waypoints, enhance_target_waypoints, \
                                m = m, measurements = measurements, min_distance = enhance_min_distance)
                        # Draw future waypoints
                        for enhance_target_waypoint in enhance_target_waypoints:
                            world.debug.draw_box(**config_waypoint_box(enhance_target_waypoint, color = (0, 255, 0)))
                        # Get enhance control
                ...
                if leave_start_position(start_pose.location, vehicle) >= 2 or tot_episodes >= 10:
                    ...
                    #enhancer.dt = clock.get_time()/1e3
                    #enhancer.steps_ahead = int(5./enhancer.dt)
                    #enhance_number_waypoints = enhancer.steps_ahead
                    #print("Update policy ehancement steps ahead {}, step length {}".format(enhancer.steps_ahead,\
                    #        enhancer.dt))
